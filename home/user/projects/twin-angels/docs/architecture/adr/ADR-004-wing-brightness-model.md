...
**劣势**：
- **存在保护失去意义**：如果代价是临时的，玩家触发暗流与否对翅膀亮度无长期影响，C5 系统的存在价值大打折扣。
- **无法实现 C5 累积曲线**：阶段基线是离散的（1.0, 0.85, 0.65, 0.35, 0.15），无法体现阶段内的渐进暗淡。
- **与 C5 GDD 设计意图冲突**：C5 明确设计为"存在保护代价是累积的"，反映"痛苦会留下痕迹"的叙事主题。
...
**劣势**：
- **失去叙事结构锚点**：翅膀亮度完全由玩家行为决定，不同玩家的翅膀亮度在同一章节可能差异巨大，破坏叙事预设的亮度曲线。
- **翅膀阶段失去意义**：C2 的 5 阶段翅膀演化是重要叙事元素，连续扣减模型无法体现阶段跳跃。
- **与 C2 GDD 设计意图冲突**：C2 明确设计了 5 阶段翅膀演化，每阶段有独立的基线和视觉表现。
- **亮度可能过早触底**：如果玩家频繁触发暗流，翅膀亮度可能在早期章节就接近 0.05，导致后期叙事无法体现"逐渐暗淡"的弧线。
...
- **所有权清晰**：C5 拥有 `wing_brightness_permanent` 和 `wing_brightness_temporary` 的写入权，C2 仅在阶段切换时通过 C5 接口重置。

**劣势**：
- 实现复杂度略高（两个变量 vs 一个变量）。
- 需要明确阶段切换时的重置逻辑。
- 需要定义代价的下限策略（见下文）。
...
2. **保留两个系统的设计意图**：
   - C2 的 5 阶段翅膀演化和阶段基线得以保留。
   - C5 的存在保护代价机制完整生效，玩家行为有长期影响。

3. **所有权清晰**：C5 ProtectionSystem 是翅膀亮度（permanent + temporary）的唯一所有者。C2 AngelSystem 通过 C5 暴露的 `update_wing_stage()` 接口在阶段切换时重置 permanent 层。这满足了主架构 §4.4 的单一所有权原则。

4. **叙事重置点自然实现**：Ch16 的叙事重置只需将 `wing_brightness_permanent` 设为 1.0，干净利落。
...
- 叙事弧线得到保障：阶段基线确保亮度跟随叙事结构，不会因玩家行为偏离预设弧线。
- 存在保护有意义：永久扣减使玩家行为有长期影响，C5 系统不是摆设。
...
- 代价平衡需要 playtest：永久扣减的速率需要通过 playtest 调整，确保玩家不会过早触底。
